...
subject = f"Рассьілка вакансий за {today}"
text_content = f"Рассьілка вакансий  {today}"
from_email = EMAIL_HOST_USER
empty = '<h2>Ксожалению на сегодня новьіх данньіх нет</h2>'
...
if users_dct:
    ...
    for keys, emails in users_dct.items():
        ...
        for email in emails:
            ...
            msg.send()
qs = Error.odjects.filter(timestamo=today)
subject = ''
text_content = ''
to = ADMIN_USER
_html = ''
if qs.exists():
    ...
    for i in data:
        _html += f'<p><a href="{i["url"]}">Error: {i["title"]}</p>'
    ...
    text_content = "Пожелания пользователя "
...
for keys in users_dct.keys():
    if keys not in urls_dct:
        if keys[0] and keys[1]:
            urls_err += f'<p>Для города {keys[0]} и язьіка {keys[1]} отсутствуют урльі</p><br>'
if urls_err:
    subject += 'Отсутствующие урльі'
    _html += urls_err
if subject:
    msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
    msg.attach_alternative(_html, "text/html")
